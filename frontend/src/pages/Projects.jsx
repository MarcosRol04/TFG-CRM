...
import {
  ...
  Card, CardContent, Avatar, Stack, alpha, useTheme
} from '@mui/material';
import {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const stats = {
        ...
        pendiente: projRes.data.filter(p => p.status === 'pendiente').length,
        en_progreso: projRes.data.filter(p => p.status === 'en_progreso').length,
        completado: projRes.data.filter(p => p.status === 'completado').length,
        cancelado: projRes.data.filter(p => p.status === 'cancelado').length
      };
      setStats(stats);
